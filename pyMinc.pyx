...
#		self.alloc_multidim_array(volume.array)
	...
	def getVolume(self):
		...
		for i in range(0,volume['dimensions']):
			volume['shape'].append(self.volume.array.sizes[i])
		...
		cdef np.ndarray data = np.zeros(np.product(volume['shape']),volume['dtype'])
		count=data.dtype.itemsize*np.product(volume['shape'])
		print count, data.dtype.itemsize, volume['shape'], volume['dtype']
		memcpy(<char*>data.data,<char*>self.volume.array.data,count/4)
#		data.shape = volume['shape']
		volume['data'] = data
	...
	def invent(self):
		...
		print voxels, size, dtype
		
		alloc_volume_data(vol)

		cdef np.ndarray arr = np.zeros(np.product(sizes),dtype)
		arr[0] = 1; arr[500] = 2; arr[999] = 3
		cdef void *dataPtr


#		vol.array.data = <void *>(arr.data)
