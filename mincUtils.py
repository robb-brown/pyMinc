...
from ORO.Extras.MINC import OROMINCReader

try:
	from . import myTempfile; from .myTempfile import *
	from . import xfmParam
except:
	import myTempfile; from myTempfile import *
	from xfmParam import *
...
class Display(object):
	...
	def __del__(self):
		for dir in self.tempdirs:
			self.tempfile.releaseTempDir(dir)
		
	
	def display(self,images,labels=None,synchronous=False):
		if not images.__class__ in [list,tuple]:
			images = [images]

		tempdir = self.tempfile.getTempDir(fast=True)
		self.tempdirs.append(tempdir)
		for i,image in enumerate(images):
			if not image.__class__ == str:
				try:
					fname = p.basename(image.getFileName())
					fname = p.join(tempdir,fname)
				except:
					fname = p.join(tempdir,'temp%d.mnc' % int(random.random()*10000))
				if image.__class__ == OROMINCDataInterface:
					OROMINCWriter.write(image,fname)
				elif image.__class__ in [numpy.ndarray,numpy.ma.core.MaskedArray]:
					im = VIOVolume(image); im.write(fname)
				elif image.__class__ in [VIOVolume]:
					image.write(fname)
				else:
					continue
		...
		if not labels==None and not labels.__class__ == str:
			fname = p.join(tempdir,'temp%d.mnc' % int(random.random()*10000))
			if labels.__class__ == OROMINCDataInterface:
				OROMINCWriter.write(labels,fname)
			elif labels.__class__ in [numpy.ndarray,numpy.ma.core.MaskedArray]:
				im = VIOVolume(labels); im.write(fname)
			elif labels.__class__ in [VIOVolume]:
				labels.write(fname)
			else:
				fname = None
...
def addCosinesToTransform(xfm,source=None,target=None):
	sourceXFM = mat(cosinesToTransform(getCosines(source))) if source is not None else mat(identity(4))
	targetXFM = mat(cosinesToTransform(getCosines(target))) if target is not None else mat(identity(4))
	xfm = mat(readXFM(xfm))
#	xfm = xfm.dot(invertXFM(sourceXFM)).dot(targetXFM)
	xfm = targetXFM*xfm*sourceXFM.I
	return array(xfm)
...
def getDisplacementVolume(transform):
	if issubclass(transform.__class__,VIOGeneralTransform):
...
def writeXFM(xfm,fname,dvol = None,dvolName=None):
	if not dvolName:
		dvolName = dvol
	...
	if dvol:
		...
		s += 'Displacement_Volume = %s;' % p.basename(dvolName)
	s += '\n'
	f = open(fname,'w')
	f.write(s)
	f.close()
	if dvol:
		if not p.exists(p.join(p.dirname(fname),p.basename(dvolName))):
			execute('cp %s %s' % (dvol,p.join(p.dirname(fname),p.basename(dvolName))))
	return fname
...
def getXFMLevel(xfm,asString=False):
	...
	if issubclass(xfm.__class__,VIOGeneralTransform):
		temp = xfm
		if not temp.linear:
			return 'nonlinear'
		xfm = temp.data
...
def reduceXFMLevel(xfm,level):
	level = xfmLevelToInteger(level)
	if level >= 100:
		return xfm
	if issubclass(xfm.__class__,VIOGeneralTransform):
		x = xfm.linearTransforms.data
	else:
		x = xfm
	
	params = xfmParameters(x)
	lev = level
	if lev >= 12:
		params.pop('shears')
	if lev >= 9:
	...
	if lev >= 3:
		params.pop('translations')
	eliminate = mat(xfmFromParameters(**params),float64).I
	x = x * eliminate
	...
	newParams = {}
	if lev >= 12:
		newParams['shears'] = params.pop('shears')
	if lev >= 9:
		newParams['scales'] = params.pop('scales')
	if lev >= 6:
		newParams['rotations'] = params.pop('rotations')
	if lev >= 3:
		newParams['translations'] = params.pop('translations')
		
	x = xfmFromParameters(**newParams)
	
	if issubclass(xfm.__class__,VIOGeneralTransform):
		xfm = VIOGeneralTransform(x)
	else:
		xfm = x
	return xfm
...
def BEAST(image,library='standard',atlas=None,atlasMask=None,quiet=True):
	if library == 'pediatric':
		beastDir = '/usr/local/minc-toolkit/share/beast-library-NIHPD/'
	elif library == 'standard':
		beastDir = '/usr/local/minc-toolkit/share/beast-library-1.0/'
	else:
		if not p.exists(library):
			print("Could not find library %s" % library)
			return None
	config = p.join(beastDir,'default.2mm.conf')
	tempdir = '/temp/'#myTempfile.mkdtemp(fast=True)
	infile = p.join(tempdir,'BEASTIn.mnc.gz'); OROMINCWriter.write(image,infile)
	outfile = p.join(tempdir,'BEASTout.mnc')
	print(atlas); print(atlasMask)
	if not atlas is None and not atlasMask is None:
		print("Using atlas")
		atlasfile = p.join(tempdir,'atlas.mnc.gz'); OROMINCWriter.write(atlas,atlasfile)
		atlasMaskfile = p.join(tempdir,'atlasMask.mnc.gz'); OROMINCWriter.write(atlasMask,atlasMaskfile)
		normalizedfile = p.join(tempdir,'normalized.mnc')
		cmd = 'volume_pol %s %s %s --noclamp --order 0 --source_mask %s --target_mask %s' % (infile,atlasfile,normalizedfile,atlasMaskfile,atlasMaskfile)
		execute(cmd,quiet=quiet)
		infile = normalizedfile

	cmd = 'time mincbeast %s %s %s -median -fill -same_resolution -conf %s' % (beastDir,infile,outfile,config)
	if not quiet:
		print("Doing BEAST with: %s" % cmd)
	execute(cmd,quiet=quiet)
	outfile2 = p.join(tempdir,'BEASTout-full.mnc')
	cmd = 'mincresample %s %s -like %s' % (outfile,outfile2,infile)
	execute(cmd)
	result = OROMINCReader.read(outfile2)
...
def BET(image,quiet=True):
	...
	h = bm.getMINCHeaders()
	for dim in h['dimensions']['order']:
		h[dim]['step'] = originalImage.getMINCHeaders()[dim]['step']
...
def removeCosines(input, output,convertToUInt=False,quiet=True):
#	print "	Removing cosines"	
	
	if (1):
		tempInput = p.join(p.dirname(output),p.basename(input).replace(input.split('.')[0],'workingRemoveCosines'))
...
#				if sum(cosines) < 0:
#					fixCosines(im,spaceRepresentative,extraArgs)			# ROBB
#					return 'BadCosines'
...
def resample(input,output=None,xfm=None,like=None,extraArgs="",convertToUInt=False,simpleResample=False,quiet=True,spaceRepresentative=None,vio=None):
	...
	try:
		if not input.__class__ in [str,unicode]:
			...
			if inputImage.__class__ == VIOVolume:
				inputImage.write(input)
			else:
				OROMINCWriter.write(inputImage,input)
		if not output:
			output = p.join(tempdir,'tempOutput.mnc')
		
		if not like is None and not like.__class__ in [str,unicode]:
			likeImage = like
			like = p.join(tempdir,'tempLike.mnc')
			if likeImage.__class__ == VIOVolume:
				likeImage.write(like)
			else:
				OROMINCWriter.write(likeImage,like)

		xfmMatrix = None
		if not xfm is None and not xfm.__class__ in [str,unicode]:
			xfmMatrix = xfm
	...
	#	print "Resampling %s like %s output: %s" % (input,like,output)
		...
		cmd = 'mincconvert -2 %(input)s %(output)s' % dict(input=input,output=p.join(tempdir,'scrap.mnc'))
		execute(cmd,quiet=quiet); input = p.join(tempdir,'scrap.mnc')
		if (like is None):
			if (xfm):
			...
			cmd = 'mincconvert -2 %(like)s %(output)s' % dict(like=like,output=p.join(tempdir,'scrapLike.mnc'))
			execute(cmd,quiet=quiet); like = p.join(tempdir,'scrapLike.mnc')
			if (xfm):
		...
		if not quiet:
			print(cmd)
		if outputImage:
			if vio == True or (vio is None and isinstance(inputImage,VIOVolume)):
				output = VIOVolume(output)
			else:
